...
namespace ImageProcessor
{
    ...
    public class ImageFactory : IDisposable
    {
        ...
        public ImageFactory CustomThreshold(int parameter)
        {
            if (this.ShouldProcess)
            {
                CustomFilter customFilter = new CustomFilter { DynamicParameter = parameter };
                this.backupFormat.ApplyProcessor(customFilter.ProcessImage, this);
            }
        ...
        public ImageFactory Crop(Rectangle rectangle)
        {
            if (this.ShouldProcess)
            {
                CropLayer cropLayer = new CropLayer(rectangle.Left, rectangle.Top, rectangle.Width, rectangle.Height);
        ...
        /// The current instance of the <see cref="T:ImageProcessor.ImageFactory"/> class.
        /// </returns>
        public ImageFactory Filter(IFilter filterType)
        {
            if (this.ShouldProcess)
            {
                Filter filter = new Filter { DynamicParameter = filterType };
