...
namespace ImageProcessor.Imaging.Helpers
{
    ...
    public static class PixelOperations 
    {
        ...
        public static Color Sepia(Color color)
        {
            var red = color.R * .393f + color.G * .769f + color.B * .189f;
            var green = color.R * .349f + color.G * .686f + color.B * .168f;
            var blue = color.R * .272f + color.G * .534f + color.B * .131f;
            if (red > 255)
                red = 255;
            if (green > 255)
                green = 255;
        ...
        {
            var intensity = (color.R + color.G + color.B) /
                            3;
            return Color.FromArgb(intensity, intensity, intensity);
        }

        public static Color Threshold(Color color, int par)
        {
            var intensity = (color.R + color.G + color.B) /
                            3;
            var newColor = intensity >= 255 * par / 100 ? Color.FromArgb(255, 255, 255) 
